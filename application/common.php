...
/**
 * 检查数据表是否存在某个字段的某个值
 * @param $model string 模型名称或模型对象
 * @param $filed string 字段
 * @param $value string 字段值
 * @return bool
 */
function checkTableExistValue($model, $filed, $value)
{
    $model = is_string($model) ? model($model) : $model;
    $result = $model->field($filed)->where($filed, $value)->find();
    return $result ? true : false;
}

/**
 * 异步返回数据
 * @param $code int 0:成功，1：失败，2：未登录
 * @param $msg
 * @param array $data
 */
 ...
 * @return mixed
 */
function getMaxMinYear()
{
    $result = model('Classes')->field('min(year) as min,max(year) as max')->find();
    $data = [
        'min' => $result->min,
        'max' => $result->max,
    ];
...
{
    $result = model('Classes')->field('semester')->where('year', $year)->order('semester DESC')->find();
