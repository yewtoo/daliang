...
class Expand extends Backend
{
    ...
    {
        parent::_initialize();
        $this->model = new \app\admin\model\Expand;
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax())
        {
            ...
            $total = $this->model
                    ->with(['category','subject','teacher','student'])
                    ->where($where)
                    ->order($sort, $order)
                    ->count();
            ...
            foreach ($list as $row) {
				...
				$row->visible(['subject']);
				$row->getRelation('subject')->visible(['name','type']);
				$row->visible(['teacher']);
				$row->getRelation('teacher')->visible(['username']);
				$row->visible(['student']);
				$row->getRelation('student')->visible(['username']);
            }
