define(['jquery', 'bootstrap', 'backend', 'table', 'form', 'template'], function ($, undefined, Backend, Table, Form, Template) {
    var Controller = {
        index: function () {
            ...
            Table.api.init({
                extend: {
                    index_url: Config.uwebadmin.api_url + '/addon/index',
                    add_url: '',
                    edit_url: '',
                    del_url: '',
                    multi_url: ''
            ...
            $(document).on("click", ".btn-userinfo", function () {
                ...
                if (!userinfo) {
                    Layer.open({
                        ...
                        area: ['430px', '350px'],
                        title: __('Login Uwebadmin'),
                        resize: false,
                        btn: [__('Login'), __('Register')],
                        yes: function (index, layero) {
                            Fast.api.ajax({
                                url: Config.uwebadmin.api_url + '/user/login',
                                dataType: 'jsonp',
                                data: {
                    ...
                    Fast.api.ajax({
                        url: Config.uwebadmin.api_url + '/user/index',
                        dataType: 'jsonp',
                        data: {
                            user_id: userinfo.id,
                        ...
                        Layer.open({
                            ...
                            yes: function () {
                                Fast.api.ajax({
                                    url: Config.uwebadmin.api_url + '/user/logout',
                                    dataType: 'jsonp',
            ...
            var install = function (name, version, force) {
                ...
                Fast.api.ajax({
                    ...
                    data: {
                        ...
                        token: token,
                        version: version,
                        faversion: Config.uwebadmin.version
                    }
                }, function (data, ret) {
                    Layer.closeAll();
            ...
            var upgrade = function (name, version) {
                ...
                Fast.api.ajax({
                    url: 'addon/upgrade',
                    data: {name: name, uid: uid, token: token, version: version, faversion: Config.uwebadmin.version}
                }, function (data, ret) {
