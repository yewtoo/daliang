...
$link = array(
    'qqun'  => "http://www.uweb.net.cn/",
    'gitee' => 'http://www.uweb.net.cn/',
    'home'  => 'http://www.uweb.net.cn/',
    'forum' => 'http://www.uweb.net.cn/',
    'doc'   => 'http://www.uweb.net.cn/',
);
...
if (is_file($lockFile)) {
    ...
    if ($open_basedir) {
        ...
        if ($dirArr && in_array(__DIR__, $dirArr)) {
            $errInfo = '当前服务器因配置了open_basedir，导致无法读取父目录<br><a href="http://www.uweb.net.cn/" target="_blank">点击查看解决办法</a>';
        }
    }
    if (!$errInfo) {
        $errInfo = '当前权限不足，无法写入配置文件application/database.php<br><a href="http://www.uweb.net.cn/" target="_blank">点击查看解决办法</a>';
    }
} else {
    $dirArr = [];
    foreach ($checkDirs as $k => $v) {
        if (!is_dir(ROOT_PATH . $v)) {
            $errInfo = '当前代码仅包含核心代码，请前往官网下载完整包或资源包覆盖后再尝试安装，<a href="http://www.uweb.net.cn/" target="_blank">立即前往下载</a>';
            break;
        }
    }
}
...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $mysqlDatabase = isset($_POST['mysqlDatabase']) ? $_POST['mysqlDatabase'] : 'uwebadmin';
    ...
    try {
        //检测能否读取安装文件
        $sql = @file_get_contents(INSTALL_PATH . 'uwebadmin.sql');
        if (!$sql) {
            throw new Exception("无法读取application/admin/command/Install/uwebadmin.sql文件，请检查是否有读权限");
        }
        ...
        <form method="post">
            ...
            <div class="form-group">
                ...
                <div class="form-field">
                    ...
                    <input type="text" name="mysqlDatabase" value="uwebadmin" required="">
                </div>
